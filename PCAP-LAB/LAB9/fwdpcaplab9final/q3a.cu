...
{
	int mthRow = threadIdx.x;

	for(int k = 0; k<n1 ; k++)// all the columns of mthRow
	{
		int temp = 0;
		for( int i = 0; i < n0 ; i++ )
		{
			temp += a[mthRow*n0 + i] * b[k + i*n1];
		}
		t[mthRow*n1 + k] = temp;
	}
...
int main() {
	int a[6]={1,1,1,1,1,1}, b[6]={1,1,1,1,1,1}, *t, m0=3, n0=2, m1=2, n1=3, i, j;
	int *d_a, *d_b, *d_t;
	...
	MatMul<<< 1, m0>>>(d_a, d_b, d_t, m0, n0, m1, n1);
