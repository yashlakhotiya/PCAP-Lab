...
{
	int nthColumn = threadIdx.x;

	for(int k = 0; k<n1 ; k++)// all the rows of nthColumn
	{
		int temp = 0;
		for( int i = 0; i < n0 ; i++ )
		{
			temp += a[k*n0 + i] * b[nthColumn + i*n1];
			// printf("%d %d: %d %d\n", mthRow, nthColumn, a[mthRow*n0 + i], b[nthColumn + i*n1]);
		}
		t[k*n1 + nthColumn] = temp;
	}
...
int main() {
	...
	MatMul<<< 1, n1>>>(d_a, d_b, d_t, m0, n0, m1, n1);
